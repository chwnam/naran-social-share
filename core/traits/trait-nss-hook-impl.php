...
if ( ! trait_exists( 'NSS_Hook_Impl' ) ) {
	trait NSS_Hook_Impl {
		 ...
		 * @param int|null                   $priority        If null, priority comes from 'NSS_PRIORITY' constant.
		...
		private function __hook_get_priority( $priority ): int {
			return is_null( $priority ) ? nss()->get_priority() : intval( $priority );
		}
	}
}

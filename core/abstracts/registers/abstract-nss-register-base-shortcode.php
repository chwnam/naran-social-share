...
if ( ! class_exists( 'NSS_Register_Base_Shortcode' ) ) {
	abstract class NSS_Register_Base_Shortcode implements NSS_Register {
		use NSS_Hook_Impl;
		...
		public function register() {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof NSS_Reg_Shortcode ) {
		...
		public function heading_actions_handler() {
			if ( is_singular() && $this->heading_actions ) {
				...
				foreach ( array_unique( $this->found_tags ) as $tag ) {
					$callback = nss_parse_callback( $this->heading_actions[ $tag ] );
		...
		public function dispatch( $atts, string $enclosed, string $tag ): string {
			$callback = nss_parse_callback( $this->real_callbacks[ $tag ] ?? '__return_empty_string' );

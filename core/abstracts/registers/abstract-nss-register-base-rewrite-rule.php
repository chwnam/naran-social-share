...
if ( ! class_exists( 'NSS_Register_Base_Rewrite_Rule' ) ) {
	abstract class NSS_Register_Base_Rewrite_Rule implements NSS_Register {
		use NSS_Hook_Impl;
		...
		public function register() {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof NSS_Reg_Rewrite_Rule ) {
					$item->register();
					if ( $item->binding ) {
		...
		public function handle_binding() {
			...
			if ( ( $binding = $this->bindings[ $wp->matched_rule ] ?? null ) ) {
				try {
					$callback = NSS_parse_callback( $binding );
				} catch ( NSS_Callback_Exception $e ) {
					wp_die(
						new WP_Error(
							'snp_rewrite_rule_error',
							/* translators: formatted callback */
							sprintf( __( 'Rewrite rule binding `%s` is invalid. Please check your rewrite rule register items.', 'nss' ), nss_format_callback( $binding ) )
						)
					);
				}
				...
				if ( apply_filters( 'nss_register_rewrite_rule_exit', true, $wp->matched_rule ) ) {
					exit;
				}
			}
		}

...
if ( ! function_exists( 'nss_cleanup_meta' ) ) {
	/**
	 * Remove plugin's meta records.
	 */
	function nss_cleanup_meta() {
		global $wpdb;
...
if ( ! function_exists( 'nss_cleanup_option' ) ) {
	/**
	 * Remove plugin's option records.
	 */
	function nss_cleanup_option() {
		global $wpdb;

		$option_names = [];

		$option = nss()->registers->option;
		if ( $option ) {
...
if ( ! function_exists( 'nss_cleanup_terms' ) ) {
	/**
	 * Remove plugin's terms.
	 */
	function nss_cleanup_terms() {
		global $wpdb;

		$taxonomies = [];

		$taxonomy = nss()->registers->taxonomy;
		if ( $taxonomy ) {
...
if ( ! function_exists( 'nss_cleanup_posts' ) ) {
	/**
	 * Remove plugin's posts.
	 */
	function nss_cleanup_posts() {
		global $wpdb;

		$post_types = [];

		$post_type = nss()->registers->post_type;
		if ( $post_type ) {

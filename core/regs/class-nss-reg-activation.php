...
if ( ! class_exists( 'NSS_Reg_Activation' ) ) {
	class NSS_Reg_Activation implements NSS_Reg {
		...
		public function register( $dispatch = null ) {
			...
			if ( $callback ) {
				if ( $this->error_log ) {
					error_log( sprintf( 'Activation callback started: %s', nss_format_callback( $this->callback ) ) );
				}

				call_user_func_array( $callback, $this->args );

				if ( $this->error_log ) {
					error_log( sprintf( 'Activation callback finished: %s', nss_format_callback( $this->callback ) ) );
				}
			}
		}
	}

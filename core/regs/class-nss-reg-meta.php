...
if ( ! class_exists( 'NSS_Reg_Meta' ) ) {
	 ...
	 */
	class NSS_Reg_Meta implements NSS_Reg {
			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new NSS_Reg_Meta( $object_type, $meta_key, $args );
				}
			...
			if ( $this->object_type && $this->get_key() ) {
				...
				try {
					if ( $this->args['auth_callback'] ) {
						$this->args['auth_callback'] = nss_parse_callback( $this->args['auth_callback'] );
					}
				} catch ( NSS_Callback_Exception $e ) {
					$error = new WP_Error();
					$error->add(
						...
						sprintf(
							...
							nss_format_callback( $this->args['auth_callback'] )
						)
					);
					wp_die( $error );
				}

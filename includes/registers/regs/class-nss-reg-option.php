...
if ( ! class_exists( 'NSS_Reg_Option' ) ) {
	/**
	 ...
	 */
	class NSS_Reg_Option implements NSS_Reg {
		...
		public static function factory( string $option_name ): ?NSS_Reg_Option {
			...
			if ( isset( $wp_registered_settings[ $option_name ] ) ) {
				if ( ! isset( static::$options[ $option_name ] ) ) {
					...
					static::$options[ $option_name ] = new NSS_Reg_Option( $args['group'], $option_name, $args );
				}
		...
		public function register( $dispatch = null ) {
			if ( $this->option_group && $this->option_name ) {
				if ( $this->args['sanitize_callback'] ) {
					try {
						$this->args['sanitize_callback'] = nss_parse_callback( $this->args['sanitize_callback'] );
						...
						$error->add(
							...
							sprintf(
								...
								nss_format_callback( $this->args['sanitize_callback'] )
							)
						);
						wp_die( $error );
					}
